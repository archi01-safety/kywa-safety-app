...
if st.button("🚀 KYWA AI 위험요인 분석 시작", use_container_width=True):
    if not api_key: 
        ...
        with st.spinner(f"KYWA AI가 {user_name} 님의 데이터를 분석 중입니다..."):
            ...
            [판정 시 주의사항]
            1. 단순 안전수칙 위반(예: 안전모 미착용)이 전체 안전 시스템의 부재(빈도 5점)를 의미하지 않음.
            2. 높이 2m 미만의 일반 사다리 작업은 강도를 2~3점으로 제한함.

[판정 예시 (Reference)]
- 예시 1: 사다리 작업 중 안전모 미착용 (높이 보통) -> 빈도 3, 강도 3 (9점, 보통)
- 예시 2: 고소 작업대 난간 미설치 -> 빈도 4, 강도 4 (16점, 매우 높음)
- 예시 3: 바닥 통로 물기 방치 -> 빈도 3, 강도 2 (6점, 낮음)

...
with col1:
    ...
    selected_facility = st.radio(
        ...
        horizontal=True, 
        key="facility_val"  # 고유 Key 설정
    )

    # 2. 담당 부서 선택 (중복 제거됨)
    st.markdown("### **📂 담당 부서 선택**")
    dept_list = [
        "활동부", "협력부", "청렴감사실", "기획혁신부", "인재경영부", 
        "홍보전략부", "안전경영부", "재무회계부", "디지털정보부", 
        "미래활동부", "정책사업부", "활동안전부", "활동인증부", 
        "청소년성장지원부", "지도인력양성부", "지도인력개발부", "자회사"
    ]
    
    selected_dept = st.selectbox(
        "부서명 선택", 
        dept_list, 
        key="dept_val"  # 고유 Key 설정
    )

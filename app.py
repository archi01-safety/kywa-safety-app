...
st.markdown("""
    ...
    div.stButton > button {
        ...
        font-weight: bold !important; border-radius: 0.5rem !important;
    }
    div.stButton > button:hover { background-color: #ff3333 !important; transform: scale(1.01); }
...
with tab1:
    ...
    with col2:
        ...
        img_file = None
        if "📸" in source_option:
            st.info("📸 [사진촬영] 또는 [사진업로드] 선택 (가로사진 권장)")
    ...
    # KOSHA GUIDE 섹션
    st.write("")
    with st.expander("📚 **관련 KOSHA GUIDE 조회 및 다운로드**", expanded=True):
    ...
    if dashboard_data is not None and not dashboard_data.empty:
        ...
        m1, m2 = st.columns(2)
        m1.metric("올해 누적 점검 건수", f"{len(yearly_data)} 건")
        m2.metric("점검결과 제출 시설", f"{yearly_data['시설명'].nunique() if '시설명' in yearly_data.columns else 0} 개 시설")

        g_col1, g_col2 = st.columns(2)
        with g_col1:
            with st.container(border=True):
                if len(yearly_data.columns) >= 5:
                    cat_col = yearly_data.columns[4]
                    st.write(f"**⚠️ {cat_col} 현황**")
                    fig_pie = px.pie(yearly_data, names=cat_col, hole=0.3)
                    fig_pie.update_layout(height=350, margin=dict(t=20, b=20, l=0, r=0))
                    st.plotly_chart(fig_pie, use_container_width=True, config={'displayModeBar': False})
        with g_col2:
            with st.container(border=True):
                if '시설명' in yearly_data.columns:
                    st.write("**🏢 시설명별 건수**")
                    fac_counts = yearly_data['시설명'].value_counts().reset_index()
                    fac_counts.columns = ['시설명', '건수']
                    fig_bar = px.bar(fac_counts, x='시설명', y='건수', color='시설명')
                    fig_bar.update_layout(height=350, margin=dict(t=20, b=20, l=0, r=0), showlegend=False)
...
f1.caption("© 2026 한국청소년활동진흥원(KYWA) 안전경영부. 본 시스템은 API 옵트아웃 정책을 준수합니다.")
f2.caption("📞 문의: 안전경영부 (02-6959-7138)")

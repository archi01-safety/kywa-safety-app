...
try:
    # 같은 디렉토리에 malgun.ttf 파일이 있어야 한글이 깨지지 않습니다.
    pdfmetrics.registerFont(TTFont(FONT_NAME, 'malgun.ttf'))
except Exception as e:
    print(f"경고: 한글 폰트를 불러오지 못했습니다. PDF에서 한글이 깨질 수 있습니다. ({e})")
    FONT_NAME = 'Helvetica' # 한글 미지원 폰트 (영문 전용)
...
def generate_risk_assessment_report(output_filename):
    ...
    # 보완된 너비 계산 (이미지 크기 28mm를 수용하도록 열 너비를 30mm로 확장, 총합 180mm 유지)
